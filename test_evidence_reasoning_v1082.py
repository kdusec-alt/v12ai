...
class EvidenceReasoningV1082Tests(unittest.TestCase):
    def test_bad_fundamental_but_strong_price_is_negative_absorbed(self):
        ...
        self.assertNotEqual(row["medium_term_bias"], "偏多")
    ...
    def test_unverified_event_cannot_become_dominant_driver(self):
        forecast = make_forecast(
            market="US",
            last=104.0,
            day_pct=4.0,
        ...
        row = build_evidence_reasoning(forecast)
        top = row["top_drivers"]
        event_rows = [item for item in top if item["category"] == "event"]
