...
This module is the shared execution/narrative arbiter for every TW/US ticker.
It does not alter formal Direction, T0/T1/High/Low, Confidence, Prediction DNA,
Auto Audit, Genome, Research weights, or calibration schedules.

Priority contract
-----------------
1. Price/Session Truth and hard data vetoes.
2. Individual selling pressure, failed structure, relative weakness and verified
   company/fundamental risk.
3. Limit-price liquidity and closed-session constraints.
4. Overextension and VWAP position.
5. Individual confirmation and only then market/event support.
6. Event waiting may cap an aggressive state, but can never promote a weak
   ticker or overwrite a red/orange state.

No ticker code, company name, sector whitelist or one-off exception is allowed
in this module. Screenshots expose generic states; they never create special
rules for the displayed ticker.
"""
...
_CLOSED_SESSIONS = {"after_hours", "after_close", "closed_reference", "closed", "休市"}
...
_LIVE_MARKERS = ("盤中", "即時", "realtime", "live", "盤前", "pre-market", "premarket")
_STALE_SESSION_MARKERS = ("前一收盤", "前收", "昨收", "previous close", "prior close")
...
def _truth_payload(decision: Mapping[str, Any], thesis: Mapping[str, Any]) -> Dict[str, Any]:
    output: Dict[str, Any] = {}
    direct = decision.get("_price_truth")
    if isinstance(direct, Mapping):
        output.update(dict(direct))
    nested = thesis.get("price_truth")
    if isinstance(nested, Mapping):
        output.update(dict(nested))
    return output
...
def _operative_values(
    decision: Mapping[str, Any], truth: Mapping[str, Any], session: str,
) -> tuple[float, float]:
...
def _price_shape(
    *, last: float, open_price: float | None, high: float | None,
    low: float | None, day_pct: float,
) -> Dict[str, Any]:
    open_to_last_pct = None
    fade_from_high_pct = None
    range_position = None
    if open_price and open_price > 0 and last > 0:
        open_to_last_pct = (last / open_price - 1.0) * 100.0
    if high and high > 0 and last > 0:
        fade_from_high_pct = (last / high - 1.0) * 100.0
    if high and low and high > low and last > 0:
        range_position = max(0.0, min(1.0, (last - low) / (high - low)))
    ...
    opening_selloff = bool(open_to_last_pct is not None and open_to_last_pct <= -2.0)
    opening_recovery = bool(open_to_last_pct is not None and open_to_last_pct >= 2.0)
    return {
        "open_to_last_pct": round(open_to_last_pct, 4) if open_to_last_pct is not None else None,
        "fade_from_high_pct": round(fade_from_high_pct, 4) if fade_from_high_pct is not None else None,
        "range_position": round(range_position, 4) if range_position is not None else None,
        "near_low": near_low,
        "near_high": near_high,
        "opening_selloff": opening_selloff,
        "opening_recovery": opening_recovery,
        "positive_day": day_pct > 0,
    }
...
def _event_context(thesis: Mapping[str, Any], decision: Mapping[str, Any], evidence: str) -> Dict[str, Any]:
    ...
    explicit_verified = event.get("verified")
    if explicit_verified is None:
        explicit_verified = thesis.get("event_verified")
    if explicit_verified is None:
        explicit_verified = thesis.get("source_verified")
    verified = bool(explicit_verified) if explicit_verified is not None else bool(
        thesis.get("event_model_eligible") or event.get("model_eligible")
    )
    if stale:
        verified = False
    ...
    elif not verified and event_wait_state:
...
def _fundamental_context(
    decision: Mapping[str, Any], thesis: Mapping[str, Any], radar: Mapping[str, Any],
) -> Dict[str, Any]:
    payloads = [
        decision.get("_earnings_intelligence"),
        decision.get("_fundamental_intelligence"),
        decision.get("_fundamental"),
        thesis.get("fundamental"),
        thesis.get("earnings"),
        radar.get("基本面"),
        radar.get("Company News"),
    ]
    text = _flatten_selected(payloads)
    state = _fundamental_text_state_cached(text.lower())
    explicit = _text(
        thesis.get("fundamental_state")
        or thesis.get("earnings_state")
    ...
    structured_verified = any(
        ...
        or str(payload.get("status") or "").lower() in {"verified", "accepted", "official"}
        for payload in structured_payloads
    )
    low_text = text.lower()
    trusted_text_source = any(
        marker.lower() in low_text
        for marker in _FUNDAMENTAL_TRUSTED_SOURCE_MARKERS
    )
    verified = bool(
        ...
        or structured_verified
        or trusted_text_source
    )
    return {
        "state": state,
        "verified": verified,
        "verification_basis": (
            "structured" if structured_verified else
            "trusted_source_marker" if trusted_text_source else
            "explicit" if thesis.get("fundamental_verified") or thesis.get("earnings_verified") else
            "unverified"
        ),
        "text": text[:500],
    }
...
def _structured_same_session(decision: Mapping[str, Any], truth: Mapping[str, Any]) -> bool:
    for payload in (
        decision.get("_session_truth_v1081"),
        decision.get("_market_session_truth"),
        truth.get("cross_asset_session_truth"),
    ):
        if isinstance(payload, Mapping):
            # "verified" may only mean that each row is real.  It does
            # not prove that every cross-asset row belongs to the same Session.
...
def _text_has_live_session(text: str) -> bool:
    low = text.lower()
    return any(marker in low for marker in _LIVE_MARKERS) and not any(
        marker in low for marker in _STALE_SESSION_MARKERS
    )


def _market_context(
    *, market: str, symbol: str, day_pct: float, session: str, evidence: str,
    decision: Mapping[str, Any], truth: Mapping[str, Any],
) -> Dict[str, Any]:
    ...
    # An individual quote saying "盤中" cannot prove that SOX/NQ/QQQ rows are
    # from the same Session.  Cross-asset confirmation requires a structured
    # Session Truth payload; free text remains context-only.
    session_payload = _mapping(decision.get("_session_truth_v1081"))
    eligible_keys = {str(key).upper() for key in (session_payload.get("eligible_keys") or [])}
    if eligible_keys:
        changes = {key: value for key, value in changes.items() if key.upper() in eligible_keys}
    same_session = _structured_same_session(decision, truth)
    session_verified = same_session

    if market == "TW":
        if str(symbol or "").upper().endswith(".TWO"):
            candidates = [changes.get("TPEX"), changes.get("TAIEX")]
        else:
            candidates = [changes.get("TAIEX"), changes.get("TPEX")]
    ...
    relative_gap = day_pct - benchmark if benchmark is not None and session_verified else None
    severe_weak = bool(
        relative_gap is not None
        and (relative_gap <= -4.0 or (benchmark >= 3.0 and day_pct < 0))
    )
    relative_strong = bool(relative_gap is not None and relative_gap >= 1.5)
    ...
    return {
        ...
        "relative_gap_pct": round(relative_gap, 4) if relative_gap is not None else None,
        "severe_relative_weakness": severe_weak,
        "relative_strength": relative_strong,
    }


def _limit_context(
    *, market: str, day_pct: float, thesis_state: str, decision: Mapping[str, Any],
    last: float, high: float | None, low: float | None,
) -> Dict[str, Any]:
    rule = _mapping(
        decision.get("_exchange_rule")
        or _mapping(decision.get("_price_meta")).get("exchange_rule")
    )
    ...
    return {
        ...
        "frozen_print": frozen_print,
        "liquidity_verified": explicit_lock is not None,
    }


def _failed_breakout(
    *, last: float, stop: float | None, vwap_position: str,
    thesis_state: str, regime_state: str,
) -> bool:
...
def _price_tiles(
    *, state: str, last: float, vwap: float | None, high: float | None,
    low: float | None, confirmation: float | None, stop: float | None,
    no_chase: float | None, first: float | None,
) -> list[Dict[str, str]]:
    vwap_text = _price(vwap)
    low_text = _price(low)
    confirm_text = _price(confirmation)
    stop_text = _price(stop)
    no_chase_text = _price(no_chase)

    if state == "BUY_TODAY_CONFIRM":
        return [
            {"label": "先鋒", "value": f"現價 {_price(last)} 小量確認"},
            {"label": "回測", "value": f"VWAP {vwap_text} 不破" if vwap else "首次量縮回測"},
            {"label": "加碼", "value": f"站穩 {confirm_text}" if confirmation else "突破平台確認"},
            {"label": "短停", "value": f"跌破 VWAP {vwap_text} 未收回" if vwap else "突破平台失守"},
            {"label": "不追", "value": no_chase_text if no_chase else "急拉過熱不追"},
        ]
    if state == "WAIT_VWAP_PULLBACK":
        return [
            {"label": "回測", "value": f"VWAP {vwap_text}" if vwap else "首次量縮回測"},
            {"label": "確認", "value": "回測不破且賣壓量縮"},
            {"label": "加碼", "value": f"站穩 {confirm_text}" if confirmation else "重新放量轉強"},
            {"label": "失效", "value": f"VWAP {vwap_text} 跌破未收回" if vwap else stop_text},
            {"label": "不追", "value": no_chase_text if no_chase else "偏離VWAP過大"},
        ]
    if state == "WAIT_VWAP_RECLAIM":
        return [
            {"label": "狀態", "value": f"先收復 VWAP {vwap_text}" if vwap else "等待結構收復"},
            {"label": "支撐", "value": low_text if low else (_price(first) if first else "低點不再下移")},
            {"label": "確認", "value": f"站回 {vwap_text} 後維持／回踩不破" if vwap else "收復後維持5–15分鐘"},
            {"label": "失效", "value": f"跌破 {low_text}" if low else (stop_text if stop else "再創盤中新低")},
            {"label": "不追", "value": "急拉碰VWAP不直接追"},
        ]
    if state == "WAIT_RECLAIM_HOLD":
        return [
            {"label": "位置", "value": f"VWAP {vwap_text} 附近" if vwap else "多空交界"},
            {"label": "確認", "value": "維持5–15分鐘或回踩不破"},
            {"label": "轉強", "value": f"站穩 {confirm_text}" if confirmation else "高點抬升"},
            {"label": "失效", "value": f"跌回 VWAP {vwap_text} 下方" if vwap else stop_text},
            {"label": "不追", "value": no_chase_text if no_chase else "量價未確認不追"},
        ]
    ...
    if state == "OVERHEATED_NO_CHASE":
        return [
            ...
            {"label": "等待", "value": f"VWAP {vwap_text}／突破平台" if vwap else "量縮回測"},
            {"label": "確認", "value": "回測後重新放量"},
            {"label": "失效", "value": f"跌破 {low_text}" if low else stop_text},
            {"label": "不追", "value": no_chase_text if no_chase else "第三段加速"},
        ]
    if state == "SELLING_EXPANSION_BLOCK":
        return [
            {"label": "進場", "value": "禁止接刀"},
            {"label": "止跌", "value": f"不再跌破 {low_text}" if low else "低點停止下移"},
            {"label": "收復", "value": f"VWAP {vwap_text}" if vwap else "重新站回關鍵均價"},
            {"label": "轉強", "value": f"站穩 {confirm_text}" if confirmation else "量價結構修復"},
            {"label": "風險", "value": stop_text if stop else (low_text if low else "再創新低")},
        ]
    if state == "FAILED_BREAKOUT_EXIT":
        return [
            {"label": "進場", "value": "取消原計畫"},
            {"label": "修復", "value": f"收復 VWAP {vwap_text}" if vwap else "站回突破平台"},
            {"label": "確認", "value": f"站穩 {confirm_text}" if confirmation else "重新放量"},
            {"label": "停手", "value": stop_text if stop else "原結構失守"},
    ...
    if state == "WAIT_NEXT_SESSION":
        return [
            ...
            {"label": "失效", "value": f"跌破 {low_text}" if low else "缺口完全回補"},
...
def assess_entry_opportunity(forecast: Any) -> Dict[str, Any]:
    ...
    open_price = _num(decision.get("開盤") or truth.get("open"))
    high = _num(decision.get("最高") or truth.get("high"))
    low = _num(decision.get("最低") or truth.get("low"))
    ...
    shape = _price_shape(
        last=last, open_price=open_price, high=high, low=low, day_pct=day_pct,
    )
    ...
    selling_expansion = bool(
        ...
        or (
            len(market_ctx["negative_proxies"]) >= 2
            and day_pct < 0
            and vwap_position == "below"
            and market_ctx["session_verified"]
        )
    )
    ...
    fundamental_veto = bool(
        fundamental["state"] == "negative"
        and fundamental["verified"]
        and (
            vwap_position == "below"
            or shape["near_low"]
            or shape["opening_selloff"]
            or market_ctx["severe_relative_weakness"]
        )
    )
    negative_absorption_watch = bool(
        fundamental["state"] == "negative"
        and fundamental["verified"]
        and not fundamental_veto
        and vwap_position in {"above", "at"}
        and day_pct > 0
    )
    severe_individual_weakness = bool(
        market_ctx["severe_relative_weakness"]
        and vwap_position == "below"
    ...
    strong_tape = bool(
        ...
        or thesis_state in _STRONG_THESIS_STATES
    ...
    event_repricing = bool(
        event["verified"]
        and event["priority"] in {"P1_IMMEDIATE_RECALC", "P2_RECALC"}
        and strong_tape
    )
    ...
    tradable_session = bool(
        session in _ACTIVE_CONFIRM_SESSIONS
        or (market == "US" and session == "pre_market")
    )
    ...
    if hard_truth_block or last <= 0:
        ...
        reasons = ["價格已跌破防守或原突破結構"]
    elif selling_expansion:
        state = "SELLING_EXPANSION_BLOCK"
        reasons = [
            "賣壓仍在擴張，且基本面存在反證"
            if fundamental_veto else "賣壓仍在擴張，跌深本身不是買點"
        ]
    elif fundamental_veto:
        state = "SELLING_EXPANSION_BLOCK"
        reasons = ["基本面反證與弱勢價格同時成立"]
    elif severe_individual_weakness:
        state = "SELLING_EXPANSION_BLOCK"
        reasons = ["大盤／產業轉強，但個股仍明顯逆勢走弱"]
    elif thesis_state in _HARD_BLOCK_STATES or (entry_permission == "blocked" and day_pct <= 0):
        state = "SELLING_EXPANSION_BLOCK"
        reasons = ["上層風險重定價或進場閘門仍未解除"]
    elif limit_ctx["limit_like"] and not limit_ctx["explicit_opened"]:
        state = "LIMIT_LIQUIDITY_WAIT"
        reasons = ["價格已到漲停附近，但封單與開板承接尚未完整驗證"]
    elif session in _CLOSED_SESSIONS:
        state = "WAIT_NEXT_SESSION"
        reasons = ["目前已非可驗證的正式盤中新進場時段"]
    elif vwap_position == "below":
        # A stock already below VWAP is not waiting for a pullback and should
        # not be hidden behind a stale no-chase threshold.  VWAP is a reclaim
        # gate here, never a direct buy price.
        state = "WAIT_VWAP_RECLAIM"
        if shape["opening_selloff"]:
            reasons = ["開高後賣壓明顯，現價仍在VWAP下方"]
        elif shape["near_low"]:
            reasons = ["現價位於VWAP下方且接近盤中低點"]
        else:
            reasons = ["價格尚未收復時段VWAP，確認門檻不是直接買價"]
    elif vwap_position == "at":
        state = "WAIT_RECLAIM_HOLD"
        reasons = ["價格位於VWAP多空交界，需要維持或回踩確認"]
    elif overextended:
        state = "OVERHEATED_NO_CHASE"
        reasons = ["方向可能正確，但漲幅或價格偏離已過度延伸"]
    elif negative_absorption_watch:
        state = "WAIT_VWAP_PULLBACK"
        reasons = ["基本面偏弱但價格暫時吸收，仍需回測確認而非直接追價"]
    elif (
        vwap_position == "above"
        and tradable_session
        and strong_tape
        and not shape["opening_selloff"]
        and not shape["near_low"]
        and fundamental["state"] != "negative"
        and (
            thesis_state in _STRONG_THESIS_STATES
            or market_ctx["relative_strength"]
            or event_repricing
            or market_ctx["confirmed"]
        )
    ):
        state = "BUY_TODAY_CONFIRM"
        reasons = ["個股價格站穩VWAP且自身結構完成轉強"]
    elif vwap_position == "above" and tradable_session:
        state = "WAIT_VWAP_PULLBACK"
        reasons = ["價格位於VWAP上方，但仍需首次量縮回測確認承接"]
    elif regime_state in _EXHAUSTION_STATES and vwap_position == "above":
        state = "BUY_TODAY_CONFIRM"
        reasons = ["賣壓已由擴張轉為衰竭，且價格收復VWAP"]
        ...
        state = "WAIT_RECLAIM_HOLD"
        reasons = ["目前優勢不足以直接追價，等待可驗證的價格結構"]

    # Event wait is a one-way safety cap.  It may downgrade a green state while
    # the verified first reaction is genuinely inside its 30-minute window;
    # it can never turn a red/orange/limit state into the generic yellow text.
    ...
    if market_ctx["relative_gap_pct"] is not None:
    ...
    price_strategy_text = "｜".join(
        f"{tile['label']} {tile['value']}" for tile in tiles[:2]
    )
    ...
    return {
        ...
        "price_strategy_text": price_strategy_text,
        "price_tiles": tiles,
        "anchor_invalidated": anchor_invalidated,
        "original_low_entry_audit": {
