...
This execution layer replaces the single-question low-entry maturity gate with
one practical decision: participate now, wait for a same-session pullback, wait
for the next trading session, avoid an overheated chase, or block a falling
knife.

It is deliberately narrative/execution-only.  It never rewrites Direction,
T0/T1/High/Low, formal confidence, Prediction DNA, Auto Audit, Genome, research
weights, or calibration schedules.  Existing low-entry prices are preserved as
model/audit facts and are merely demoted when verified session price discovery
has moved the tradable reference far away from the old anchor.
"""
...
_ACTIVE_CONFIRM_SESSIONS = {"intraday", "close_confirm"}
...
def _proxy_changes(text: str) -> Dict[str, float]:
    """Read only explicitly displayed proxy changes; missing data stays missing."""
...
def _limit_like(market: str, day_pct: float, thesis_state: str) -> bool:
    """Only a Taiwan limit-like return or the structured thesis may declare it.

    A random old headline containing the word 漲停 must never lock the current
    ticker into WAIT_NEXT_SESSION.
    """
    if thesis_state == "limit_breakout":
        return True
    return market == "TW" and day_pct >= 9.5


def _failed_breakout(
    *, last: float, stop: float | None, above_vwap: bool,
...
def _live_session_values(
    decision: Mapping[str, Any], truth: Mapping[str, Any], session: str,
) -> tuple[float, float]:
    """Promote verified live-session price truth without overwriting the model.

    The formal close remains stored for Audit.  For pre-market/intraday/
    after-hours execution, the accepted current-session quote and return are the
    operative facts; this prevents a 149 formal close from hiding a 160.41
    after-hours repricing.
    """
    live = bool(truth.get("live_session_quote"))
    truth_last = _num(truth.get("current_price"))
    truth_return = _num(truth.get("current_return_pct"))
    ...
    last = truth_last if active and live and truth_last is not None else decision_last or truth_last or 0.0
...
def _dynamic_reference(
    *, state: str, last: float, vwap: float | None,
    confirmation: float | None, first: float | None,
    second: float | None, stop: float | None,
    no_chase: float | None, anchor_invalidated: bool,
) -> Dict[str, Any]:
    original = {
        "first": first,
        "second": second,
    ...
    return {
        "primary": primary,
        "secondary": secondary,
        "anchor_status": "session_repriced" if anchor_invalidated else "original_anchor_active",
...
def assess_entry_opportunity(forecast: Any) -> Dict[str, Any]:
    ...
    last, day_pct = _live_session_values(decision, truth, session)
    ...
    above_vwap = bool(
        truth.get("vwap_available") and vwap is not None and last >= vwap
    ) or "VWAP 上方" in vwap_text or "VWAP上方" in vwap_text
    ...
    limit_like = _limit_like(market, day_pct, thesis_state)
    strong_tape = bool(
        day_pct >= (3.0 if market == "US" else 4.0)
        or thesis_state in _STRONG_THESIS_STATES
        or a_breakout
    )
    cross_market_repricing = bool(strong_tape and above_vwap and cross_market_confirmed)

    old_anchor_far = bool(first is not None and first > 0 and last > 0 and (last / first - 1.0) >= 0.07)
    old_no_chase_crossed = bool(no_chase is not None and last >= no_chase)
    anchor_invalidated = bool(cross_market_repricing and (old_anchor_far or old_no_chase_crossed))
    ...
    tradable_confirmation_session = bool(
    ...
    elif thesis_state in _EVENT_WAIT_STATES:
        state = "WAIT_INTRADAY_PULLBACK" if session not in _CLOSED_SESSIONS else "WAIT_NEXT_SESSION"
        reasons = ["事件首輪價格反應尚未完成，先等15–30分鐘量價結構"]
    elif failed and not selling_expansion:
        state = "FAILED_BREAKOUT_EXIT"
        reasons = ["價格已跌破防守／突破結構，取消原進場計畫"]
    elif selling_expansion or thesis_state in _HARD_BLOCK_STATES or (
        entry_permission == "blocked" and day_pct <= 0
    ):
    ...
    elif session in _CLOSED_SESSIONS and (strong_tape or anchor_invalidated or overextended):
        state = "WAIT_NEXT_SESSION"
        reasons = ["收盤後／盤後已形成重新定價，正式盤結構仍待驗證"]
    elif overextended:
        state = "OVERHEATED_NO_CHASE"
        reasons = ["方向可能正確，但相對大盤／產業代理已過度延伸"]
    elif cross_market_repricing and tradable_confirmation_session and (
        a_breakout or thesis_state in _STRONG_THESIS_STATES
    ):
        state = "BUY_TODAY_CONFIRM"
        reasons = ["價格站上 VWAP，且至少兩項跨市場代理同向"]
        if anchor_invalidated:
            reasons.append("原低接／不追錨點已被跨市場重新定價降級")
    elif strong_tape and above_vwap and tradable_confirmation_session:
        state = "WAIT_INTRADAY_PULLBACK"
        reasons = ["方向轉強，但跨市場確認或價格換手尚未完整"]
    elif exhaustion and above_vwap and tradable_confirmation_session:
        state = "BUY_TODAY_CONFIRM"
    ...
    conditions.append({
        "ok": above_vwap,
        "text": "價格站在時段 VWAP 上方" if above_vwap else "價格尚未站回時段 VWAP",
    })
    conditions.append({
    ...
    if anchor_invalidated:
        conditions.insert(0, {"ok": True, "text": "原低接錨點已因跨市場重新定價降級"})
